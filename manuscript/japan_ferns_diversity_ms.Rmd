...
```
By adding phylogenetic information, we are able to overview the diversity distribution patterns by a different measure from species number. We updated the rbcL sequence dataset to cover `r percent(Ntip(japan_pterido_tree)/nrow(repro_data))` of the native taxa by making several additions and corrections to that prepared by Ebihara et al. (2010) (Electronic supplementary material 1). We then calculated Phylogenetic Diversity (PD; Faith 1992, 2006) for each grid cell by trimming the tree to only taxa present in the cell and summing the branchlengths (excluding the root). In the analysis with a dataset covering all ferns and lycophyte taxa (Fig. 1b), the highest PD value was obtained in a grid in Wakayama Prefecture (grid code: `r pd_pteridos$site[1]`), the second highest in the northern part of the Okinawa Island (grid code: `r pd_pteridos$site[2]`) and third highest in Iriomote Island (grid code: `r pd_pteridos$site[3]`) located in the southwesternmost part of the archipelago. These rankings were not significantly changed when we analyzed a dataset with only fern occurrences rooted by a lycophyte (Lycopodium clavatum var. nipponicum) (Fig. 1c). In the result of PD analyses, the Yakushima grid (grid code: 453034) with maximum species richness was ranked `r which(pd_pteridos$site == "453034")`th in the PD analysis using ferns+lycophytes, and `r which(pd_ferns$site == "453034")`th in that only using ferns, respectively. The disparity of the results might suggest the difference in their constituent species – species richness in Yakushima Island is resulted from presence of some species rich genera (e.g. 19 taxa of Dryopteris, 16 taxa of Diplazium, 14 taxa of Athyrium).
...
Our updated occurrence data supported the trend that species with seasonal-green phenology showed wider distribution than evergreen species suggested by Guo et al. (2003), but it is probably an artifact caused by small land areas at lower altitudes in the country where evergreen species dominate. Guo et al. (2003) also suggested species with multiple reproductive modes showed wider distribution areas than species with a single reproductive mode. However, our data did not support this trend (mean numbers of grids, sex.+apo. = `r cps_by_repro_means_inline %>% filter(repro == "sex_apo") %>% pull(stat)`, sex. = `r cps_by_repro_means_inline %>% filter(repro == "sexual") %>% pull(stat)`, apo. = `r cps_by_repro_means_inline %>% filter(repro == "apomictic") %>% pull(stat)`, none of the combination differs significantly; analysis of variance, *p* = `r cps_by_repro_model_summary %>% filter(term == "reproductive_mode") %>% pull(p.value) %>% round_t(2)`) – the result seems to be strongly affected by species circumscription and presumably some “species with multiple reproductive modes” in Guo et al. (2003) are separated into multiple taxa with each single mode in present treatment. Even though their distribution area sizes are not significantly different, apomictic taxa show significantly narrower latitudinal range than sexual ones (mean degree of ranges: sex. = `r lat_by_repro_means_inline %>% filter(repro == "sexual") %>% pull(stat)` and apo. `r lat_by_repro_means_inline %>% filter(repro == "apomictic") %>% pull(stat)`, *p* = `r lat_by_repro_tukey %>% filter(comparison == "sexual-apomictic") %>% pull(adj.p.value) %>% round_t(3)`; Tukey's HSD), and it coincides with the result by Tanaka et al. (2014) that represented smaller range sizes of apomicitic ones both in latitude and elevation. Also the southerly distribution phenomenon of apomicitic species which was presumed a result of poor cold-hardiness of gametophytes of apomictic species having larger cell sizes (Tanaka et al. 2014), was confirmed in present data (Fig. 1d, e). The trend that narrower distribution of polyploid taxa (Guo et al. 2003) is not supported by present data excluding apomictic species (average number of grids, sexual diploid = `r cps_by_ploidy_means_inline %>% filter(sexual_diploid == TRUE) %>% pull(stat)`, sexual polyploid = `r cps_by_ploidy_means_inline %>% filter(sexual_polyploid == TRUE) %>% pull(stat)`) – the trend probably came out by narrower distribution of apomictic species which are dominated by triploid.
